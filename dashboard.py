...
<body><header><div><h1>M5 Device Telemetry</h1><div class='sub'>Live health monitor for your edge device</div></div></header><div id='dashboard'>{content}</div><div class='card'><div class='label'>LIVE HISTORY · HEAP / WI-FI</div><canvas id='historyChart' height='220'></canvas><div class='sub'><span style='color:#70f0d0'>● Free heap (KB)</span> &nbsp; <span style='color:#ffc857'>● Wi-Fi RSSI (dBm)</span></div></div><footer>Live via WebSocket · MQTT: devices/+/health</footer><script>
...
function drawChart(){const w=canvas.clientWidth||700,h=220,dpr=devicePixelRatio||1;canvas.width=w*dpr;canvas.height=h*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);const left=48,right=w-42,top=18,bottom=h-28;ctx.font='12px system-ui';ctx.strokeStyle='#294052';ctx.lineWidth=1;for(let i=0;i<=4;i++){const y=top+i*(bottom-top)/4;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(right,y);ctx.stroke();ctx.fillStyle='#8fa6b8';ctx.fillText(String(320-i*80),5,y+4);ctx.fillText(String(-20-i*20),right+7,y+4)}if(chartData.length<2)return;function line(key,min,max,color){ctx.beginPath();chartData.forEach((x,i)=>{const px=left+i*(right-left)/(chartData.length-1),py=bottom-(x[key]-min)/(max-min)*(bottom-top);i?ctx.lineTo(px,py):ctx.moveTo(px,py)});ctx.strokeStyle=color;ctx.lineWidth=3;ctx.lineJoin='round';ctx.lineCap='round';ctx.stroke()}line('heap',0,320,'#70f0d0');line('rssi',-100,-20,'#ffc857')}
