...
<body><header><div><h1>M5 Device Telemetry</h1><div class='sub'>Live health monitor for your edge device</div></div></header><div id='dashboard'>{content}</div><div class='card'><div class='label'>LIVE HISTORY</div><canvas id='historyChart' height='110'></canvas></div><footer>Live via WebSocket · MQTT: devices/+/health</footer><script src='https://cdn.jsdelivr.net/npm/chart.js'></script><script>
const chart=new Chart(document.getElementById('historyChart'),{type:'line',data:{labels:[],datasets:[{label:'Free heap (KB)',data:[],borderColor:'#70f0d0',backgroundColor:'#70f0d033',yAxisID:'heap',tension:.3},{label:'Wi-Fi (dBm)',data:[],borderColor:'#ffc857',yAxisID:'rssi',tension:.3}]},options:{animation:false,scales:{heap:{position:'left',grid:{color:'#294052'}},rssi:{position:'right',grid:{drawOnChartArea:false}}},plugins:{legend:{labels:{color:'#eaf2f8'}}}}});
function updateChart(d){chart.data.labels.push(new Date(d.received_at*1000).toLocaleTimeString());chart.data.datasets[0].data.push(Math.round(d.free_heap/1024));chart.data.datasets[1].data.push(d.wifi_rssi);if(chart.data.labels.length>30){chart.data.labels.shift();chart.data.datasets.forEach(x=>x.data.shift())}chart.update()}
